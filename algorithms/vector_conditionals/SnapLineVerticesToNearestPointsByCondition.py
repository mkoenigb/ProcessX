...
class SnapLineVerticesToNearestPointsByCondition(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        self.addParameter(
            QgsProcessingParameterFeatureSource(
                self.SOURCE_LYR, self.tr('Source Layer (Lines)'), [QgsProcessing.TypeVectorLine]))
        self.addParameter(
    ...
    def createInstance(self):
        return SnapLineVerticesToNearestPointsByCondition()

    def name(self):
        return 'SnapLineVerticesToNearestPointsByCondition'

    def displayName(self):
        return self.tr('Snap Line-Vertices to Nearest Points by Condition')
    ...
    def shortHelpString(self):
        return self.tr(
        'This algorithm snaps the vertices of a given line layer (singleline or multiline) to the nearest point of a given point layer by optional attribute and distance conditions. The point layer is casted to a singlepoint layer first, which means that the geometry of its nodes are used.'
